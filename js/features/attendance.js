...
function renderAttendanceMatrix() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const isFuture = !isInactive && isFutureDay(d);
  ...
  targetStudents.forEach((std, idx) => {
    ...
    for (let d = 0; d < daysInMonth; d++) {
      ...
      const isFuture = !isInactive && isFutureDay(dayNumber);
      // Key uses year_monthIdx_dayIdx for multi-year support
      const key = `${std.id}_${year}_${monthIdx}_${d}`;
      // Also check legacy key format (without year) for backwards compatibility
      const legacyKey = `${std.id}_${monthIdx}_${d}`;
      const currentStatus = state.attendance[key] || state.attendance[legacyKey] || "";
      const isMissed = isMissedUncheckedDay(dayNumber, currentStatus);
      ...
      if (isInactive) {
        ...
        const tooltipText = isMissed ? "ለዚህ ቀን ቀሪ ሪፖርት አልተሞላም።" : "";
...
function cycleAttendanceState(bubbleElement, stdId, dayIdx) {
  ...
  if (isInactiveDay(dayNumber)) return;
  if (isFutureDay(dayNumber)) return;
  ...
  const current = state.attendance[key] || state.attendance[legacyKey] || "";
  ...
  // Always write to new key format; remove legacy key if present
  if (state.attendance[legacyKey] !== undefined) {
    delete state.attendance[legacyKey];
  }
  if (newStatus === "") {
  ...
  if (newStatus === "✓") {
    ...
    displaySymbol = "ፍ";
  } else if (isMissedUncheckedDay(dayNumber, newStatus)) {
...
async function markAllStudentsPresent() {
  ...
  // Only allowed on today's date
  if (!isCurrentMonth()) {
    showAlert("ማሳሰቢያ", `ሁሉንም 'መጣ' ማለት የሚቻለው ለዛሬ ቀን ብቻ ነው (${getTodayDisplayString()})።`);
    return;
  }
  ...
  targetStudents.forEach((std) => {
    const key = `${std.id}_${year}_${monthIdx}_${dayIdx}`;
