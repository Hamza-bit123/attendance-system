...
function initFirstOpenedDay() {
  ...
  state.firstOpenedDay = parseInt(localStorage.getItem("medresa_first_opened_day")) || 1;
}
...
function handleInstructorReg(e) {
  ...
  if (state.editingInstructorId) {
    const ins = state.instructors.find((i) => i.id === state.editingInstructorId);
    if (ins) {
...
function showFutureDayAlert() {
  showAlert(
    "ማሳሰቢያ",
    "ይህ ቀን ገና ያልደረሰ በመሆኑ መገኘት መቆጣጠር አይቻልም።"
  );
}
...
function renderAttendanceMatrix() {
  ...
  for (let d = 1; d <= DAYS_IN_MONTH; d++) {
    ...
    const isInactive = isTodayMonth && d < state.firstOpenedDay;
    const isFuture = isTodayMonth && d > currentDay;
  ...
  targetStudents.forEach((std, idx) => {
    ...
    for (let d = 0; d < DAYS_IN_MONTH; d++) {
      ...
      const isInactive = isTodayMonth && d + 1 < state.firstOpenedDay;
      ...
      if (isFuture) {
        ...
        const tooltipText = "ይህ ቀን ሲስተሙ ከመከፈቱ በፊት የነበረ በመሆኑ መገኘት መቆጣጠር አይቻልም።";
        tableHtml += `<td class="attendance-cell inactive-day-col"><div class="${bubbleClass}" title="${tooltipText}" onclick="showInactiveDayAlert()">${displaySymbol}</div></td>`;
      } else {
...
function cycleAttendanceState(bubbleElement, stdId, dayIdx) {
  ...
  if (isTodayMonth && dayIdx + 1 < state.firstOpenedDay) {
    ...
    for (let pageIdx = 0; pageIdx < totalPages; pageIdx++) {
      ...
      studentSlice.forEach((std, sIdx) => {
        ...
        for (let d = 0; d < DAYS_IN_MONTH; d++) {
          const key = `${std.id}_${CURRENT_MONTH_INDEX}_${d}`;
          const status = state.attendance[key] || "";
          const isInactive = d + 1 < state.firstOpenedDay;
          const inactiveStyle = isInactive ? "background-color: #f8fafc; color: #94a3b8;" : "";

          if (status === "✓") {
            cells += `<td style="color:#059669; font-weight:500; ${inactiveStyle}">✓</td>`;
          } else if (status === "X") {
            cells += `<td style="background:#fee2e2; color:#dc2626; font-weight:bold; ${isInactive ? "opacity: 0.6;" : ""}">✕</td>`;
          } else if (status === "Ref") {
            cells += `<td style="background:#fef3c7; color:#d97706; font-weight:bold; ${isInactive ? "opacity: 0.6;" : ""}">ፍ</td>`;
          } else {
      ...
      for (let d = 1; d <= DAYS_IN_MONTH; d++) {
        ...
        const isInactive = d < state.firstOpenedDay;
        const inactiveStyle = isInactive ? "background: #f8fafc !important; opacity: 0.55;" : "";
